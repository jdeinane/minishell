...
/*   Updated: 2024/01/31 16:00:20 by jubaldo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_strchr(cmd, '/') == NULL && get_env_var_index(data->env, "PATH") == -1)
		return (error_msg(cmd, NULL, "command not found", CMD_NOT_FOUND));
	if (access(cmd, F_OK) != 0)
		return (error_msg(cmd, NULL, "No such file or directory", CMD_NOT_FOUND));
